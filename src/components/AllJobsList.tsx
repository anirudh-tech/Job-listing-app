...
export default function AllJobsList() {
  ...
  const fetchAllJobs = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/jobs?status=${statusFilter}&page=${page}&limit=${limit}`);
  ...
  const markApproved = async (jobId: string) => {
    try {
      ...
      console.error('Error marking job as approved:', e);
                ...
                <tr key={job._id} className="border-t">
                  ...
                  <td className="px-4 py-3 text-gray-700">
                    <div className="flex flex-col gap-1">
                      ...
                      {job.subcategory && (
                        <span className="text-xs text-purple-600">{job.subcategory}</span>
                      )}
                    </div>
                  </td>

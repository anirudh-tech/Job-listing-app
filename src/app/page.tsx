...
function Home() {
  ...
  const [jobSeekersError, setJobSeekersError] = useState('');
  ...
  const fetchJobSeekers = async () => {
    try {
      ...
      const response = await fetch('/api/job-seekers?page=0&limit=6');
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.error || 'Failed to fetch job seekers');
      }
      const items = Array.isArray(data) ? data : data?.items || [];
      setJobSeekers(items.slice(0, 6));
      setJobSeekersError('');
    } catch (error) {
      console.error('Error fetching job seekers:', error);
      setJobSeekersError('Unable to load job seekers right now.');
    } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main id="about" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch mb-6">
            <Reveal variant="left">
              <div className="w-full rounded-xl overflow-hidden border border-gray-100 bg-white">
                <img src="https://res.cloudinary.com/daob5eure/image/upload/v1761156708/womensafety_lzyf6x.jpg" alt="Women safety" className="w-full h-64 object-cover" />
              </div>
            </Reveal>
            <Reveal variant="right">
              <div className="rounded-2xl text-white p-6 sm:p-8 min-h-[16rem]" style={{ backgroundColor: '#0b4383' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch mb-6">
            <Reveal variant="left">
              <div className="order-2 lg:order-1 rounded-2xl text-white p-6 sm:p-8 min-h-[16rem]" style={{ backgroundColor: '#0b4383' }}>
            ...
            <Reveal variant="right">
              <div className="order-1 lg:order-2 w-full rounded-xl overflow-hidden border border-gray-100 bg-white">
                <img src="https://res.cloudinary.com/daob5eure/image/upload/v1761156708/jobopportunities_yufr3o.jpg" alt="Job opportunities" className="w-full h-64 object-cover" />
              </div>
            </Reveal>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch mb-6">
            <Reveal variant="left">
              <div className="w-full rounded-xl overflow-hidden border border-gray-100 bg-white">
                <img src="https://res.cloudinary.com/daob5eure/image/upload/v1761156708/teachingtostudents_hcufuk.jpg" alt="Morality teaching" className="w-full h-64 object-cover" />
              </div>
            </Reveal>
            <Reveal variant="right">
              <div className="rounded-2xl text-white p-6 sm:p-8 min-h-[16rem]" style={{ backgroundColor: '#0b4383' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch mb-6">
            ...
            <Reveal variant="right">
              <div className="order-1 lg:order-2 w-full rounded-xl overflow-hidden border border-gray-100 bg-white">
                <img src="https://res.cloudinary.com/daob5eure/image/upload/v1761156709/Education_Upliftment_yc3urq.jpg" alt="Education upliftment" className="w-full h-64 object-cover" />
              </div>
            </Reveal>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
            <Reveal variant="left">
              <div className="w-full rounded-xl overflow-hidden border border-gray-100 bg-white">
                <img src="https://res.cloudinary.com/daob5eure/image/upload/v1761156708/saveNature_tzwtbz.jpg" alt="Save nature" className="w-full h-64 object-cover" />
              </div>
            </Reveal>
            <Reveal variant="right">
              <div className="rounded-2xl text-white p-6 sm:p-8 min-h-[16rem]" style={{ backgroundColor: '#0b4383' }}>
        ...
        <section id="job-seekers" className="mt-14">
          <div className="text-center mb-8">
            <p className="text-gray-500">Here You Can See</p>
            <h3 className="text-3xl font-extrabold text-gray-900">
              Latest <span style={{ color: '#0b4383' }}>Job Seekers</span>
            </h3>
          </div>
          {jobSeekersLoading ? (
            ...
            <div className="bg-white border border-red-200 text-red-600 rounded-lg px-4 py-6">
              {jobSeekersError}
            </div>
          ) : jobSeekers.length === 0 ? (
            <div className="bg-white border border-dashed border-gray-300 rounded-lg px-6 py-10 text-center text-gray-600">
              No job seekers have registered yet. Check back soon!
            </div>
          ) : (
            <>
              <div className="grid gap-6 md:grid-cols-2">
                {jobSeekers.map((seeker) => (
                  <div key={seeker._id} className="bg-white border border-gray-100 rounded-xl shadow hover:shadow-md transition p-6 flex flex-col gap-3">
                    <div>
                      <h4 className="text-lg font-semibold text-gray-900">{seeker.name}</h4>
                    ...
                    <div className="mt-auto pt-4 text-xs text-gray-500 border-t border-gray-100">
                      Registered on {formatRegisteredDate(seeker.createdAt)}
                    </div>
                  </div>
                ))}
              </div>
              <div className="text-center mt-8">
                <a
                  href="/job-seekers"
                  className="inline-block px-6 py-3 rounded-md border border-blue-600 text-blue-600 hover:bg-blue-50"
                >
